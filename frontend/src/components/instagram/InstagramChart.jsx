// src/components/instagram/InstagramChart.jsx
...
import { impressionsData } from "./data/impressionsData";
import { savesData } from "./data/savesData";
              ...
              <div className="absolute top-full mt-2 right-0 w-full bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl z-10">
                {chartConfigs.map((config) => (
                  <button
                    ...
                    className={`w-full text-left px-6 py-3.5 text-white hover:bg-pink-500/20 transition-all ${
                      selectedChart === config.type ? "bg-pink-500/20" : ""
                    }`}
                  >
                    {config.label}
                  </button>
              ...
              <div className="absolute top-full mt-2 right-0 w-full bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl z-10">
                {filters.map((filter) => (
                  <button
                    ...
                    className={`w-full text-left px-6 py-3.5 text-white hover:bg-pink-500/20 transition-all ${
                      selectedFilter === filter.value ? "bg-pink-500/20" : ""
                    }`}
                  >
                    {filter.label}
                  </button>
